...
describe('verify the graphql apis', function () {
    it.skip('allToDo', function () {
        cy.request({
            ...
            // expect(res.body.data.allTodos.length).to.eql(2)

        })
    })

    it.skip('addToDO', function () {
        cy.request({
            method: "POST",
            url: "http://localhost:3000/",
            body: {
            ...
            //cy.log(response)
            expect(response.body.data.createTodo).have.ownProperty('id')
        })
    })

    it.skip('UpdateToDO', function () {
        cy.request({
            method: "POST",
            url: "http://localhost:3000/",
            body: {
