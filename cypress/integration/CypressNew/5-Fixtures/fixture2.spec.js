...
describe('Describe ways to use the fixture file', function () {
    ...
    it('Read the fixture file in before', function () {
        ...
        cy.get('.feedback-input').first().type(data.firstName)
        cy.get('.feedback-input').eq(1).type(data.lastName)
        cy.get('input[name="email"]').type(data.email)
        cy.get('.feedback-input').last().type(data.message)
